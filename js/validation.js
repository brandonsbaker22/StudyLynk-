...
function capitalizeFirst(field) {
  var val = field.value;
  if (val.length > 0) {
    // Title-case the value: first letter uppercase, rest lowercase
    field.value = val.charAt(0).toUpperCase() + val.slice(1).toLowerCase();
  }
}
...
function isValidZip(zip) {
  // Accept 5-digit or ZIP+4 (12345 or 12345-6789)
  var pattern = /^\d{5}(?:-\d{4})?$/;
...
function clearError(fieldId) {
  ...
  if (field) {
    ...
    // Remove aria-invalid entirely when clearing the error
